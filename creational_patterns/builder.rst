...
注：Director和Builder是聚合关系，Director由Builder组成。
...
建造者模式可以用于描述KFC如何创建套餐：套餐（注：上边提到的builder）是一个复杂对象，它一般包含主食（如汉堡、鸡肉卷等）和饮料（如果汁、
可乐等）等组成部分，不同的套餐有不同的组成部分，而KFC的服务员（注: 上边提到的director）可以根据顾客的要求，一步一步装配这些组成部分，构造一份完整的套餐，然后返回给顾客。

.. image:: /_static/KFCBuilder.jpg

注： KFCWaiter和MealBuilder之间是聚合关系也是关联关系，KFCWaiter由MealBuilder组成且KFCWaiter知道MealBuilder，但是MealBuilder不知道KFCWaiter。
优点
--------------------
...
注： 建造者模式适用情况包括：需要生成的产品对象有复杂的内部结构，这些产品对象通常包含多个成员属性并且这些产品对象具有较多的共同点；需要生成的产品对象的属性相互依赖，需要指定其生成顺序；对象的创建过程独立于创建该对象的类；隔离复杂对象的创建和使用，并使得相同的创建过程可以创建不同类型的产品。
